...
from vescale.dtensor._ops._common_rules import pointwise_rule
...
"""
all tensor ops are kept as they need to support RaggedShard.
"""
...
def equal_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    for arg_strategy in select_strategy.strategies:
        ...
        if is_tensor_partial(arg_spec):
            ...
            equal_strategy.strategies.append(PlacementStrategy(output_specs=output_spec))
        else:
...
def create_like_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    for arg_strategy in select_strategy.strategies:
        ...
        create_like_strategy.strategies.append(PlacementStrategy(output_specs=output_spec, input_specs=(arg_spec,)))
...
def new_factory_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    for arg_strategy in input_strategy.strategies:
        ...
        if tuple(input_shape) == tuple(output_shape) and input_spec.is_sharded():
            ...
            if op_schema.op == aten.new_empty_strided.default and not is_tensor_evenly_shardable(
...
def gen_bucketize_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    for arg_strategy in input_strategy.strategies:
        ...
        bucketize_strategy.strategies.append(
            PlacementStrategy(output_specs=arg_spec, input_specs=(arg_spec, replica_spec))
        )
...
def gen_slice_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    if not slice_strategy.strategies:
        ...
        for arg_strategy in input_strategy.strategies:
            ...
            slice_strategy.strategies.append(PlacementStrategy(output_specs=unshard_spec))
...
def gen_slice_scatter_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    for arg_strategy in input_strategy.strategies:
        ...
        if not (is_tensor_dim_sharded(arg_spec, dim=slice_dim) or is_tensor_partial(arg_spec)):
            ...
            slice_scatter_strategy.strategies.append(PlacementStrategy(output_specs=arg_spec))
    ...
    if not slice_scatter_strategy.strategies:
        ...
        for arg_strategy in input_strategy.strategies:
            ...
            slice_scatter_strategy.strategies.append(PlacementStrategy(output_specs=replicate_spec))
...
def scatter_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    op_strategy = expand_to_full_mesh_op_strategy(mesh, op_schema, single_mesh_dim_strategies, inplace_op=inplace_op)
    return op_strategy
...
def gather_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    return expand_to_full_mesh_op_strategy(mesh, op_schema, single_mesh_dim_strategies, input_index=1)
...
def _derive_follow_placements_from_tuple_strategy(
    ...
    for arg_strategy in tuple_strategy.childs:
        ...
        if arg_strategy.mesh != mesh:
            raise ValueError(f"All operands in {op} must have the same mesh, but got {arg_strategy.mesh} and {mesh}.")
...
def stack_strategy(op_schema: OpSchema) -> StrategyType:
    ...
    follow_placements = _derive_follow_placements_from_tuple_strategy(op_schema.op, input_tuple_strategy)
...
def prop_index_select(op_schema: OpSchema) -> OutputSharding:
    ...
    all_indices_spec: list[DTensorSpec | None] = [indices_spec if dim == i else None for i in range(values_spec.ndim)]
...
def split_rule(op_schema: OpSchema) -> OutputSharding:
    ...
    if input_spec.sums:
        raise NotImplementedError(
            f"splitting distributed tensor with Partial placement is not implemented!\nDTensorSpec={input_spec}"
        )
